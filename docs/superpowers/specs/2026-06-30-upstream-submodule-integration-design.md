...
2. 构建副本从 `sources/<BUILD_DIR>` 同步得到。
3. `wrt_core/update.sh` 对构建副本执行 feeds 更新、包替换、主题设置、patch 应用和系统配置修改。
...
- 生成 `metadata/sources/<BUILD_DIR>.lock`，记录源码目录、远程地址、分支和 commit。
- 在 CI 中使用 lock 信息参与缓存 key 和构建摘要。

## GitHub Actions 改造

`build_wrt.yml` 与 `release_wrt.yml` 需要同步更新：

- `actions/checkout` 启用 `submodules: recursive`。
...
2. 添加源码准备、校验、锁定和补丁导出脚本。
3. 改造 `build.sh`，让本地源码副本成为默认构建入口。
4. 改造 GitHub Actions checkout、缓存、校验和产物验证。
5. 更新 README，说明本地源码修改、submodule 更新和 CI 构建流程。
