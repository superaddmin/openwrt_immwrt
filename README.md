...
脚本会先复用 `./build.sh <device> debug` 准备 `action_build/`，再执行 OpenWrt 原生 `make tools/install`、`make toolchain/install` 与 `make target/compile` 预热 host 工具、工具链和目标内核产物，最后只对指定包执行 `make package/.../compile`。产物输出到 `ipk_artifacts/<device>/`。
